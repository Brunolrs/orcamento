...
export function filterAndRender() {
    ...
    // 2. Atualiza Input de Orçamento (Meta) com formatação
    ...
    if (budgetInput) {
        if (view === "ALL") {
            ...
            // Formata para PT-BR visualmente
...
function renderListsAndCharts(transactions, currentIncome) {
    ...
    if (budgetEl) {
        ...
        if (view === "ALL" || currentBudget === 0) {
            ...
            // Verde se positivo, Vermelho se estourou
    ...
    Object.keys(grouped).sort().forEach(cat => {
        ...
        // Cabeçalho da Categoria com Valor Total
        ...
        items.forEach(item => {
            ...
            if (appState.isEditMode) {
                ...
                // DATA FATURA (Lógica Corrigida)
                let dateInvISO;
                if(item.invoiceDate) {
                    // Se já tem data de fatura salva, usa ela
                    const [di, mi, yi] = item.invoiceDate.split('.');
                    dateInvISO = `${yi}-${mi}-${di}`;
                } else {
                    // SE NÃO TIVER DATA FATURA, USA O MÊS DA CONTA (billMonth) + DIA 10
                    // Isso garante que a data sugerida esteja no mês correto da fatura
    ...
    // Botão Salvar Flutuante (Apenas modo edição)
    if (appState.isEditMode) {
